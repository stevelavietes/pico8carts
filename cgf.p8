 ...
 return {
  ...
  update=function(t)
   ...
   if next_cell.x != 0 or next_cell.y != 0 then
    next_cell = vecadd(
     next_cell,
     vecmake(g_player_piece.grid_x, g_player_piece.grid_y)
    )
    g_board:shift_cell_from(g_player_piece.grid_x, g_player_piece.grid_y, next_cell.x, next_cell.y)
   end
   -- @}
 ...
 return {
  ...
  shift_cell_from=function(t, from_i, from_j, to_i, to_j)
   ...
   if (
    ...
    if not t.all_cells[to_i][to_j] then
     cls()
     print(from_i.." "..from_j)
     print(to_i.." "..to_j)
     stop()
...
function make_level()
 ...
 g_board = add_gobjs(make_board(14,16))
